...
extension Date {
    func isSameDate(_ withDate: Date) -> Bool {
        let order = Calendar.current.compare(self, to: withDate, toGranularity: .day)
        return order == .orderedSame
    }
    
    func isBeforeDate(_ withDate: Date) -> Bool {
        let order = Calendar.current.compare(self, to: withDate, toGranularity: .day)
        return order == .orderedAscending
    }
    
    func isAfterDate(_ withDate: Date) -> Bool {
        let order = Calendar.current.compare(self, to: withDate, toGranularity: .day)
